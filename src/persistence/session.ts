  ...
  try {
    ...
    if (slots.current) {
      ...
      const resumeNotice = seconds
        ? `${formatDuration(seconds)} of protected research time restored${caughtUp.window?.remainingTicks ? '; the eight-hour offline cap was reached' : ''}. Tank water and fish development kept going; health is not simulated yet.`
        : '';
