...
export function BreedingPlanner({ fish, tanks, goal, onGoal, motherId, fatherId, onMother, onFather }: Props) {
  ...
  const initialSpecies: Species = selectedMother?.species === selectedFather?.species ? selectedMother?.species ?? 'koi' : selectedMother?.species ?? selectedFather?.species ?? 'koi';
  ...
  const candidates = useMemo(() => sortCollection(fish.filter(f => f.status === 'living' && !isEgg(f.life)
    && f.species === species && (scope === 'all' || f.tankId === scope) && `${f.name} ${f.id}`.toLowerCase().includes(query.toLowerCase())), species === 'koi' && goal ? 'goal' : 'name', species === 'koi' ? goal : null), [fish, scope, query, goal, species]);
  const mother = candidates.find(f => f.sex === 'F'), father = candidates.find(f => f.sex === 'M');
  const chooseSpecies = (next: Species) => {
    setSpecies(next);
    if (selectedMother?.species !== next) onMother('');
    if (selectedFather?.species !== next) onFather('');
  };
  ...
  return <div className="breeding-planner">
    <div className="planner-scope"><label>Species<select aria-label="Breeding species" value={species} onChange={e => chooseSpecies(e.target.value as Species)}><option value="koi">Koi</option><option value="axolotl">Axolotl</option></select></label></div>
    {species === 'koi' ? <>
    <div className="planner-heading"><strong>Build your koi breeding goal</strong><span>Up to four equally weighted traits</span></div>
    ...
    </> : <div className="planner-heading"><strong>Breed axolotls</strong><span>Independent 66-locus inheritance · morphology, pigment, pattern and behavior previewed below</span></div>}
    ...
    <div className="planner-heading"><strong>{candidates.length} eligible {species === 'koi' ? 'koi' : 'axolotls'}</strong>{species === 'koi' ? <button disabled={!goal || !mother || !father} onClick={() => { onMother(mother!.id); onFather(father!.id); }}>Use both goal leaders</button> : null}</div>
    <div className="planner-parents">{(['F', 'M'] as const).map(sex => {
      ...
      const rawSelected = fish.find(f => f.id === selectedId), selected = rawSelected?.species === species ? rawSelected : undefined;
      const available = candidates.filter(f => f.sex === sex), leader = available[0];
    ...
    {species === 'koi' && selectedMother?.species === 'koi' && selectedFather?.species === 'koi' && traits.some(t => GOAL_BY_KEY.get(t.descriptor)?.locus) ? <div className="planner-odds"><strong>What this pair can pass on</strong>{traits.map(t => {
    ...
    {species === 'koi' && selectedMother?.species === 'koi' && selectedFather?.species === 'koi' && !isAxolotlGenome(selectedMother.genome) && !isAxolotlGenome(selectedFather.genome)
      ? <OffspringPrediction mother={selectedMother.genome} father={selectedFather.genome} goals={traits.map(t => t.descriptor)} /> : null}
    {species === 'axolotl' && selectedMother?.species === 'axolotl' && selectedFather?.species === 'axolotl'
      ? <AxolotlOffspringPrediction mother={selectedMother.genome} father={selectedFather.genome} /> : null}
    <p className="help-copy">{species === 'koi' ? 'Match ranks expressed adult traits, not offspring odds. Target copies reveal hidden carriers; a blend can match both colors. Combined goals average their scores. Leaders are ranked individually; review the pair’s pedigree F below.' : 'Axolotls use their own genome and linkage map. Koi breeding goals and koi allele odds are never applied to them; review the axolotl-specific offspring ranges for this pair instead.'} Filtering keeps your selected parents.</p>
  </div>;
}
