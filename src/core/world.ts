...
export function createWorld(timestamp: string, seed = 481516): World {
  const world: World = { version: 1, seed, nextId: 1, credits: 1200, fish: [], tanks: [
    { id: 'tank-1', name: 'The Koi Garden', capacity: 60, planted: true },
    { id: 'tank-2', name: 'Breeding Studio', capacity: 60, planted: false },
  ] };
...
export function applyCommand(world: World, command: Command): World {
  ...
  switch (command.type) {
    ...
    case 'breed': {
      ...
      if (next.fish.length + COHORT_SIZE > MAX_RECORDS) throw new Error('The lab save has reached its 1,000-record limit. Export your save before starting another experiment.');
    ...
    case 'buy': {
      ...
      if (next.fish.length >= MAX_RECORDS) throw new Error('The lab save has reached its record limit.');
    ...
    case 'add-tank':
      ...
      next.tanks.push({ id: `tank-${next.tanks.length + 1}`, name: `Lineage Tank ${next.tanks.length + 1}`, capacity: 60, planted: true });
      break;
    case 'decorate': {
