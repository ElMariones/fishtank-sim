...
const schema = z.discriminatedUnion('version', [
  z.object({ version: z.literal(1), ...header, tanks: z.array(z.object(tank)).min(1).max(MAX_TANKS), fish }),
  z.object({ version: z.literal(2), ...header, tanks: z.array(z.object({ ...tank, water })).min(1).max(MAX_TANKS), fish }),
]);

/** World v1 predates the water model (FS-301): each of its tanks starts with default, clean, oxygen-saturated water. */
export function decodeSave(raw: string): World {
  ...
  const world: World = parsed.version === 2 ? parsed : { ...parsed, version: 2, tanks: parsed.tanks.map(entry => ({ ...entry, water: defaultWater() })) };
