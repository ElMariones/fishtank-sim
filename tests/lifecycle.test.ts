...
const labCross = (tankId: string, timestamp = NOW, motherId = 'FSH-000001', fatherId = 'FSH-000002'): Command => ({ type: 'breed', motherId, fatherId, tankId, timestamp, genomeVersion: 3 });
...
describe('FS-406 batch rehoming and clutch selection', () => {
  it('moves a batch in one command and rejects any invalid batch without change', () => {
    ...
    const reserved = applyCommand(applyCommand(world, labCross('tank-1', LATER, 'FSH-000003', 'FSH-000004')),
      { type: 'pair', motherId: 'FSH-000001', fatherId: 'FSH-000002', nurseryId: 'tank-2', size: 24, timestamp: NOW, genomeVersion: 3 });
  ...
  it("groups a pair's offspring by clutch while the Parents filter keeps the whole pair", () => {
    ...
    let courting = applyCommand(createWorld(NOW), { type: 'pair', motherId: 'FSH-000003', fatherId: 'FSH-000004', nurseryId: 'tank-2', size: 16, timestamp: NOW, genomeVersion: 3 });
