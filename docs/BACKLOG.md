...
| ID | Pri | Days | Role | Depends | Task and acceptance |
|---|---|---:|---|---|---|
| FS-601 | DONE | 3 | Genetics/persistence | FS-405 | Data-driven registry and genome-v3 migration; old fish retain their original expression version. DONE, pushed `fb6b593`; [evidence](research/FS-601-REGISTRY-AND-GENOME-V3.md) |
| FS-602 | DONE | 4 | Rendering/genetics | FS-601 FS-306 | Validated tail topology + barbel/dorsal variants; reachable fixtures and no invalid geometry. DONE, pushed `0e95249`; [evidence](research/FS-602-STRUCTURE-ANATOMY.md) |
| FS-603 | DONE | 2 | Genetics | FS-602 | Mutation-origin propagation and save-local carrier counts with declared scope. DONE, pushed `1d0d85c`; [evidence](research/FS-603-MUTATION-ORIGINS.md) |
| FS-604 | DONE | 3 | Genealogy/UI | FS-603 FS-404 | Named bloodline registry; ancestry contribution and standard similarity shown separately. DONE, pushed `d9f17c0`; [evidence](research/FS-604-BLOODLINE-REGISTRY.md) |
| FS-605 | DONE | 3 | Design/QA | FS-604 | Multi-generation unusual-line demonstration; mutation discovery pacing and tradeoffs reviewed. DONE, pushed `bb10985`; [evidence](research/FS-605-UNUSUAL-LINE.md) |

## M7 — 20 days

| ID | Pri | Days | Role | Depends | Task and acceptance |
|---|---|---:|---|---|---|
| FS-701 | DONE | 4 | Rendering/runtime | FS-505 FS-605 | Canvas vs PixiJS spike and chosen renderer cache/LOD; record device, frame timings and memory. DONE, pushed `abf16d5`; renderer unchanged, decision recorded. [evidence](research/FS-701-RENDERER-SPIKE.md) |
| FS-702 | DONE | 3 | QA/simulation | FS-701 | 100-generation and 2000-owned-fish soak; no invalid records, leaks or duplicate events. DONE, pushed `b8fde77`; 151 generations / 2030 records with zero problems, and command cost reaching 85 ms at 2000 records recorded for its own ticket. [evidence](research/FS-702-SOAK.md) |
...
| FS-117 | DONE | Aquascape overhaul: drag-and-drop editor on the live tank, 26 procedurally drawn plants, rocks, woods and ornaments with shape variants, substrate/backdrop/lighting looks, nine themes and a renderer with caustics, light rays, lighting grades, bubbles and glass. Evidence: [TESTING](TESTING.md#fs-117-aquascape-overhaul). |
| FS-119 | DONE | Axolotl follow-up (user request): persistent axolotl shop counter (world v14) with founders, pigment morphs and documented carriers; price model 2 values axolotls within their species; readable axolotl trait cards with hidden-carrier and recorded-only notes; hover/focus locus notes on both genome views. Evidence: [TESTING](TESTING.md#fs-119-axolotl-shop-pricing-and-locus-notes). |
| FS-120 | DONE | Axolotl anatomy v2 and locomotion overhaul (user request): articulated tapered limbs, curved membranes, feathery animated gills, finer skin/pigment rendering, breathing and traveling tail wave, alternating steps/swimming sweep, deterministic bottom exploration and scale-dependent detail. Acceptance: extreme animated bounds and food pursuit tests, pigment/pose review, purchase/select/feed/pause/reload browser journey and measured 60-animal render pass. Evidence: [TESTING](TESTING.md#fs-120-axolotl-anatomy-and-locomotion). |
| FS-121 | DONE | Fast breeding switch (user request): an on/off Breeding option that makes the selected pair breed instantly, lets parents live in different tanks, and lets the player choose the spawn tank and egg count. Acceptance: core tests for cross-tank breeding, destination, count, skipped readiness checks, remaining blockers and 20-egg replay default; browser cross-tank journey and reload persistence. Evidence: [TESTING](TESTING.md#fs-121-fast-breeding-switch). |
| FS-118 | DONE | Instant tank switching: persistent motion worker and render loop, shared and idle-warmed scene layers, tiled substrates, deferred collection with queued cached portraits, and delayed loading indicators on the tank, collection and rail. Evidence: [TESTING](TESTING.md#fs-118-tank-switching). |
