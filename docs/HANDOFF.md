...
> FS-303 utility behavior and the compound breeding/Genome 2 improvements are DONE, pushed as `e7aefc1`. Continue with FS-304 spatial hashing and shelter/obstacle footprints. Read GDD §8, architecture §7 and the FS-202 and FS-302 reports. Keep behavior in the motion worker as transient visual state, and separate desire selection from steering. Expose each fish's current state and reason to the inspector. Behavior must not be saved or change biology, and visual motion speed must not alter persistent biology. M1 FS-111 pooled five observers (54/60); the unanimously missed markings trial-9 is a follow-up, not a blocker.
