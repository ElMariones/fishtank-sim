...
**Newest continuation (22 September 2026): FS-122–127 competition circuit and calendar.** Read [COMPETITIONS.md](COMPETITIONS.md) and [design/COMPETITION_UI.md](design/COMPETITION_UI.md). World v15 adds `circuit`; runtime/save migration and three competition ledger reasons are in place. `src/ui/Competitions.tsx` owns board, registration, exhibit/ceremony/results, negotiation and honors. `App.tsx` advances explicit calendar days and `session.ts` no longer applies offline catch-up. The core historical catch-up helper remains for research/tests. 317 tests in 49 files and production build pass; browser evidence is in [TESTING.md](TESTING.md#competition-circuit-and-calendar). Events are local NPC competitions, not multiplayer. Archive cap 240 is explicit and never discards awards. Balance values still need playtesting. Existing M7 backlog remains open. Leave unrelated `.claude/` files untouched.

Latest work: **FS-702 soak** (M7), DONE, pushed `b8fde77`. `src/core/soakScenario.ts` reaches 151 generations and 2030 records through live commands with zero problems, and found that `applyCommand`'s `structuredClone` makes a command cost about **85 ms in a 2000-record save** (2.0 ms at ~80 records). That is a scaling cost, not a leak, and replacing the deep copy is a command-layer change rather than a QA fix, so it is recorded for its own ticket. See [FS-702 evidence](research/FS-702-SOAK.md). Before that: **FS-701 renderer spike** (M7), DONE, pushed `abf16d5`. It ships no renderer change: the aquarium draws exactly as before, and `drawFish`/`drawAxolotl` only gained an optional trailing `detail` argument defaulting to full detail, with a test asserting an identical draw-op stream to renderer v7 across all 83 fixtures. Its finding is that **Canvas 2D costs ~0.25–0.32 ms per fish**: 2.9 ms for a 12-adult tank and 19.0 ms (about 53 fps, fish only) for the 60-place maximum aquarium on the measured device — tight at the extreme, not broken. The user's call was to leave the renderer as it is. M7's gate needs an acceptance matrix on named hardware, and neither exists yet; FS-704 sets them. A detail-dropping LOD tier and a full-detail sprite cache were both measured and rejected, and PixiJS (20–40× faster, identical pixels) is recorded as the escape hatch with a concrete trigger. See [FS-701 evidence](research/FS-701-RENDERER-SPIKE.md) and ADR-072. FS-702 is next. Previously: **FS-120 axolotl anatomy/locomotion overhaul**. `axolotlAnatomy.ts` is presentation version 2; `axolotlPose.ts` is the pure transient breathing/gill/limb/tail deformation layer. Behavior v3 adds an axolotl-only bottom bias and push/glide rhythm. Genome/expression/world versions are unchanged. The 286-test/build run and isolated browser journey pass; see [FS-120 testing](TESTING.md#fs-120-axolotl-anatomy-and-locomotion). Fine detail is scale-dependent; preserve the separate pigment-layout and contour RNG streams when adjusting LOD. M7 FS-701 remains the next roadmap task.
...
- **Live commands only:** `soakScenario.ts` must keep sending real commands through `executeCommand` and never edit a world directly, like the other scenario harnesses. It is evidence, and a harness that reached into the world would prove nothing.
- **Records, not living fish:** `SOAK_RECORDS` counts fish records. Living fish are capped at `MAX_LIVING` (480), so any "N fish" target above that can only mean records.
- **Keep the pair alive:** both phases retire the outgoing parents only once both replacements exist, which is what keeps living fish at two while records grow. Rehoming the parents unconditionally makes an all-one-sex clutch end the run; never rehoming them fills the tank and the cross fails on capacity.
- **The cost curve is a regression guard:** `costCurve` pins command cost growth to roughly proportional in the record count. If a change makes it steeper, that is a defect, not a slower machine — re-measure before adjusting the bound.
- **Slow on purpose:** the run takes about 15 s. Do not shorten the defaults to speed the suite; the state being checked only exists after a hundred generations.
