...
| Generated names (ADR-055) | The pools hold at least 200 prefixes, 500 nouns and 200 suffixes, with no duplicates. Of 10,000 names drawn against a growing taken set, all are distinct, deterministic, at most 32 characters long and free of repeated words. Single nouns run out and reroll into longer names. Without avoidance, over 99% of multi-part names are still unique. In one world, the shop, a lab cross, a bought newcomer, a bought listing, a day-3 delivery and a 20-egg clutch get generated names that are all distinct, and the world round-trips. A world v7 journal saved without `naming` replays with numbered names, rebases onto model 2 with an empty journal and names later births under model 2, and that runtime then replays exactly. Pre-v7 records-only checks ignore names; tampering with a tank ID is still refused |
