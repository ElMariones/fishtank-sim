...
- `npm run check`: **286 tests in 45 files**, strict TypeScript and production build pass. The existing Vite main-chunk advisory remains (732.98 kB, gzip 237.45 kB). `node scripts/check-docs.mjs` and `git diff --check` pass.
- Five new pose/behavior tests cover animated extreme silhouettes over 80 phases in both swimming and grounded modes, attached gills/toes, repeatability and immutable source anatomy, non-finite motion inputs, deterministic bottom settlement and leaving the bottom to eat. The existing 1,200-anatomy sweep, genetics, prices, migration and replay suites continue to pass.
- `scripts/verify-axolotl.cjs` runs a fresh isolated Chrome context against a Vite server (default port 5191). It buys two axolotls, selects a collection portrait, inspects Genome and Overview, feeds, verifies changing canvas pixels during playback and identical pixels when paused, and reloads to verify the purchase persists. Chrome **153.0.8010.50**, viewport 1440 × 1000, no page errors. It never reads or modifies the user's browser save.
- Visual iterations reviewed morphology and pigment sheets, eight swimming/grounded phases, and the live mixed tank/inspector. Follow-up passes replaced circular patches with irregular contours, separated far feet, gave limbs tapered volume, reduced excessive outlines, and tuned substrate height. Smooth membranes, pigment granules, costal grooves, lidless eyes and a restrained mouth replace the previous flat presentation. Gills stay attached while swaying independently.
- The final synthetic render workload draws 60 seeded adult phenotypes at size 38 for 35 frames, discards five warm-up frames, and reports **7.8 ms median / 8.1 ms p95** for 30 measured Canvas command passes. The first pass before detail culling measured 17.7 / 161 ms. Fine pinnules appear above 90 body-unit pixels; closely spaced filaments and tiny pigment contours simplify below 65. Marking placement uses a separate RNG from outline detail, so detail level cannot rearrange pigment anchors. This is a local command-submission microbenchmark, not end-to-end FPS, GPU timing or FS-701 completion.
- Reproducible local evidence: `.artifacts/axolotl-verification.json`, `axolotl-morphs.png`, `axolotl-animation.png`, and `axolotl-tank.png`; generated artifacts are intentionally Git-ignored. Run with `PLAYWRIGHT_MODULE` pointing to the installed Playwright package and optionally set `FISHTANK_URL`.

Real-animal reference: [San Diego Zoo's axolotl overview](https://animals.sandiegozoo.org/animals/axolotl) informed feathery external gills, the retained tail/body fin and lidless eyes. Colors, unusual anatomy, locomotion timing and genetics remain game abstractions. No claim of photorealism, physically solved foot contact, terrain climbing, real gill physiology or external playtest approval is made.
