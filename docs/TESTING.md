...
**Latest recorded run:** FS-504, 15 September 2026, Windows 11, Node 22.18.0, npm 10.9.3, in-app Chromium (Claude desktop browser pane) on isolated `localhost:5182`. 186 tests in 31 files and the strict production build pass. A fresh world completed the seven-step guide through the real UI. It then spent down to ◈ 0 and rehomed every male, and the koi rescue gave a free male who could court at once. 390 px layout checked. See [FS-504 evidence](research/FS-504-ONBOARDING-AND-RECOVERY.md).
