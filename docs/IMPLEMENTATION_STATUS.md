...
## Current continuation — FS-702 soak passing, one scaling defect recorded (awaiting push)
...
"2000 owned fish" is read as 2000 *records*. Living fish are capped by design at 480 (8 tanks × 60 places); the record set — living plus sold plus rehomed — is what actually grows in a long save.

**One defect found and deliberately not fixed here.** `applyCommand` begins with `structuredClone(world)`, which is what makes a rejected command unable to leave the world half-changed. The cost is that every command copies every record: measured at 2.0 ms per command at ~80 records, 22.5 ms at 806 and **84.9 ms at 2006**. The growth is close to proportional, so it is a scaling cost rather than a leak, but a 2000-record save hitches on every action and the schema allows five times as many records again. Replacing the deep copy with structural sharing is a command-layer change, not a QA task, so it is recorded for its own ticket with the measured curve as its before-figure.

Verification: **304 tests / 47 files**, strict TypeScript and the production build pass. The soak run itself takes about 15 s, taking the suite from roughly 11 s to 27 s. See [FS-702 evidence](research/FS-702-SOAK.md) and [testing](TESTING.md#fs-702-soak).

## Previous continuation — FS-701 renderer spike DONE, pushed `abf16d5`, renderer unchanged
...
The headline is not the renderer choice. **Canvas 2D costs about 0.25–0.32 ms per fish**: 2.9 ms for the 12-adult tank a player usually watches, and 19.0 ms — roughly 53 fps for the fish layer — for the 60-place aquarium the shop sells after two paid upgrades, before the aquascape, plants, caustics and glass are drawn. That is tight at the extreme of the current design, not broken, and the decision is to leave the renderer alone. M7's gate is an acceptance matrix on named hardware, and neither the matrix nor the hardware is chosen yet, so this measurement is the first number to put into it rather than a pass or a fail. FS-704 owns setting it.
...
## Previous continuation — FS-120 axolotl anatomy and locomotion overhaul DONE (user request)
...
- **Blind axolotl purchase:** the shop sold one unseen axolotl whose sex and genome followed the next record number. World v14 adds a persistent axolotl counter: four listed founders, pigment morphs (priced by rarity: ◈ 300/360/440) and documented carriers (◈ 340), with portraits and details, on the koi delivery rhythm. `buy-axolotl-listing` is atomic like `buy-listing`; save validation re-derives each price and carrier from the genome.
- **Axolotl sale prices:** koi buyer thresholds made almost every bred axolotl a prize (all are under the miniature keeper's 40 cm, and the tail ratio read as a long koi fin). Price model 2 judges axolotls within their species; model 1 journals replay unchanged.
- **Axolotl genome view:** carrier badges broke the three-column locus grid; they now sit with the locus name. Allele tooltips name the allele. Trait cards read in words, list hidden carriers and mark unsimulated traits (maturity, regeneration, feeding drive) as recorded only.
- **Locus notes:** hover or focus any locus in the koi or axolotl genome view for what it does and how its copies combine.
- **Evidence:** 281 tests in 44 files, strict TypeScript and the production build pass; browser journey on the live dev save. See [TESTING](TESTING.md#fs-119-axolotl-shop-pricing-and-locus-notes).

## Previous continuation — independent axolotl species DONE (user request)

18 September 2026: started from `d0fd3b9`, equal to `origin/main`.

- **Species contract / world v13:** every persistent animal and clutch carries `species: 'koi' | 'axolotl'`. Worlds v1–v12 migrate existing records as koi without changing their historic genome arrays, names, seeds, IDs or pedigree. Axolotl genomes are separately discriminated and validated; species/genome, pedigree and clutch mismatches reject.
- **Axolotl Genome v1:** 66 axolotl-only loci on 11 synthetic chromosomes with phased linked inheritance, crossover, species-local mutation targets/rates, founder distributions and deterministic expression. It covers adult size; body/head/neck/snout/jaw; separate fore/hind limbs and digits; tail/fin; external gills; eyes; skin; melanophore/xanthophore/iridophore systems; recessive leucistic-like/albino-like/melanoid-like anchors plus axanthic-like and synthetic polygenic outcomes; patterns, behavior, life history and regeneration. Koi loci/registry are never reused.
- **Rendering/development:** a standalone side-profile axolotl renderer and anatomy builder draws the broad head, four limbs/toes, long finned tail and external feathery gills. Morphology extremes are sanitized/bounded; picking, portrait framing and live selection use axolotl geometry. Juveniles keep proportionally larger heads/eyes/gills, shorter developing limbs/tails and progressive pigment reveal.
- **Mixed tanks:** koi and axolotls share the same tank/care/capacity and collision world. Behavior uses the same deterministic worker but schooling/social attraction is species-local. Cross-species breeding is rejected atomically in instant and normal breeding; delayed clutches persist their species.
- **Names/UI:** axolotls have a separate deterministic naming vocabulary keyed to expressed morph, color, pattern, gills, size, limbs/tail and temperament. Collection filters, inspector phenotype cards, 66-locus chromosome browser and shop founder purchase are species-aware. The breeding planner filters parents by species; koi prediction rejects axolotl genomes and axolotl pairs get their own 256-sample offspring preview.
- **Persistence compatibility:** axolotl mutation records persist their stable species-local `locusId`; early v13 records that lost it recover the ID from the immutable locus order, while koi mutation JSON stays unchanged. Runtime replay/save round-trips are deterministic.
- **Evidence:** `npm run check` passes **271 tests in 43 files** plus strict TypeScript and the Vite production build. Dedicated coverage includes 18 axolotl genetics tests, 6 renderer/anatomy tests, 11 integration/save tests, 4 species-boundary tests and 7 breeding-planner boundary tests. An isolated headless-Chrome run on port 5190 bought two axolotls into the koi tank, inspected the 66-locus genome, switched the breeding planner to axolotls and rendered the 256-sample axolotl offspring preview with no page errors on reload. See [TESTING](TESTING.md#independent-axolotl-species--world-v13).

## Previous continuation — FS-118 instant tank switching DONE, pushed `d0fd3b9` (user request)
...
- **Layout:** the aquarium view is rebuilt around the live tank, with a navigation rail (aquarium cards with capacity and water tone, tools, save indicator), a top bar with animated stat pills, a compact notice strip, a glass status overlay on the tank, Collection/Breeding/Care/Habitat tabs, drawers for the shop, buyers and saves, toasts and a sticky inspector.
- **Look:** lagoon palette, hover and press motion, caustics and bubbles, reduced-motion support, and responsive icon-rail and phone layouts. Commands, saves and core models are unchanged.
- **Evidence:** 218 tests and the build pass. The aquarium page shrank from 6,201 px to 3,786 px with the tank above the fold at 1,280 px. See [TESTING](TESTING.md#fs-116-ui-overhaul).

## Previous continuation — FS-605 DONE, pushed `bb10985`
...
- FS-603 adds:
  - **Origins:** every recorded mutation is an origin; children inherit it only through the transmitted homolog, traced beside the existing meiosis draws.
...
- FS-505 adds:
  - **Research → Paid economy:** six seeded keepers play 90 game days with live commands only. It reports sources and sinks checked against the ledger, loop milestones and a no-softlock route every 15 game days. The FS-504 route now lives in `src/core/playtest.ts`.
  - **Best-first batch sales (ADR-059):** reviews sell the highest offers first and the command pays exactly the review. A 40-fish fixture pays ◈ 697 instead of ◈ 536.
  - **Loop fixes:** **Select all with a buyer N**; guide completion explains selling and rehoming surplus; a 60-place aquarium explains why expansion is disabled; Research tabs wrap on phones.
...
- FS-504 adds:
  - **First-session guide:** seven device-local steps from the GDD's first session with **Show me** pointers. Steps complete from real actions or world facts and stay complete.
  - **World save v9 and the koi rescue:** one unrelated adult of each missing sex at no cost when credits cannot buy stock, then a 10-game-day wait. An aquarium alert and **If credits run low** in Buyers and ledger explain every free option.
...
- Followed by FS-505 (above).

## Previous continuation — FS-503 DONE, pushed `f78d007`

Paid aquariums and expansions, a keyboard-accessible decoration editor, persisted transforms and shared steering footprints are implemented. World v8 preserves v7 shop stock, water, ledger and existing tank layouts. `npm run check`: 174 tests in 29 files pass; strict production build passes with the main-bundle size advisory. Browser purchase, expansion, placement, rejection, reload, free edit/removal, low-credit, ledger and 390 px checks pass on isolated port 5181. See [FS-503 evidence](research/FS-503-HABITAT-EXPANSION.md).

Historical next task was FS-504; now delivered above. FS-505 must measure the paid expansion economy; the existing E-05 experiment still uses free research tanks.

## Previous continuation — truthful trait names DONE, pushed `314c477` (user request, ADR-056)

Naming model 3: a name never claims a trait the fish lacks. Neutral words hold no color, pattern, size, shape, behavior, age, lineage or sex claims. Trait words need tags from the fish's sex and expressed adult phenotype: a single expressed body color, a drawn accent, patterns, eyes, scales, sheen, size potential, tail, shape, appetite, movement and inherited tendencies. About half the fish with a nameable trait get one trait part. Older world v7 saves are compared without names during replay, then rebase onto model 3, keeping stored names. `npm test`: 168 tests in 28 files pass, and the strict production build passes. In the browser pane on isolated port 5176, the model 2 QA save loaded with no console errors and kept names such as "Rocket". A lab cross laid eggs whose inspector rows matched their names: "Stripey Grouper Splashworth" has faint tiger stripes, "Platy Blacktip" faint dark fin tips, "Skittish Bingo" Boldness 10, and "Regent Kazoo the Fiery" a classic orange accent. That fish's gold and slate blend body is named for neither color.

Historical next task was FS-503; now delivered above.

## Previous continuation — generated fish names (user request, ADR-055), pushed `f38c2ce`
...
Historical next task was FS-503, now delivered above. M5 remains open. `.claude/launch.json` remains untracked.
...
- FS-504: `onboarding.ts` with the seven-step **First session guide** and its **Show me** pointers; world save v9 with `recovery.ts`, the `claim-relief` koi rescue, the missing-sex alert and **If credits run low** in Buyers and ledger; free fixes on unaffordable care warnings; parent links with sibling and offspring counts under every fish's name. See [FS-504 evidence](research/FS-504-ONBOARDING-AND-RECOVERY.md).
...
| Breeding | Normal breeding enforces adult stage, condition, rest days, a shared tank and a reserved nursery, with recorded pause reasons; the opt-in Fast breeding switch (instant cross, any tanks, chosen destination and count) still bypasses them as a labeled shortcut; no courtship animation, mate preference, natural breeding or cost | FS-906 |
...
| Decorations | Shared cover/rock footprints and body-center clearance; extreme fins can overlap; persisted position, scale and rotation; no collision mesh or drag placement | FS-701 |
| Economy | Economy model v1: five NPC buyers with bounded, recovering demand and explained offers; founders resell below the stock price; a reconciled ledger; free rehoming. Persistent shop stock has fixed specimens, filters, expiry and ledger-backed purchases (FS-502). Two starter tanks are included; extra tanks, expansions and new decorations are paid. Breeding and food are free; no upkeep, collector orders or real market. The koi rescue (FS-504) gives one unrelated adult of each missing sex when credits cannot buy stock, at most once every 10 game days. FS-505 found no recurring sink after the build-out and no return on premium care in healthy tanks | FS-705 |
