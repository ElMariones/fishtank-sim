...
## Current continuation — FS-503 verified, awaiting push
...
**Continue M5 with FS-504: first-session onboarding, no-money recovery and discoverable family inspection.** FS-503 is locally verified; see its evidence and push status above. Preserve the fixed shop specimens, nursery reservations and atomic ledger-backed purchases. Keep sale commands carrying a price model, migrations writing keys in schema order, and the ledger reconciled. FS-403 prediction is available early; it does not establish M4's two-generation gate.
