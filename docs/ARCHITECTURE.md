...
`core/aquascape.ts` holds the append-only piece catalog (kind, mount, radius, price), look options, `settle`, `styleCost` and themes; `core/aquascapeLayout.ts` holds `placePiece`, `nearestValid` and `themeLayout`. `tankManagement.ts` derives radii and prices from the catalog and reports `layoutProblem` with the pieces involved. `rendering/decor.ts` draws each piece procedurally from its item and variant; `rendering/aquascape.ts` paints the scene (cached static layer, caustic patterns and glass overlay; per-frame plants in back and front passes, bubbles, rays, grade and lantern glow) and exposes `pieceBox`, which the editor overlay uses for grabbing. `TankCanvas` takes an optional draft `preview` scene; the motion worker still receives only saved footprints. `ui/AquascapeEditor.tsx` provides the overlay and dock. ADR-066.
