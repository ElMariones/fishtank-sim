...
- **Workspace:** the top calendar advances a Day, Week or Month and reports the resulting care and life changes. Notices (calendar report, guide, recovery) sit in a compact strip, then the live tank. A glass overlay shows motion state, oxygen, ammonia, stocking, feeding, temperature and a care-warning button that opens the Care tab. Motion and **Feed** sit on the glass, with ambient caustics and rising bubbles.
...
- **Bloodline registration (FS-604):** **Register bloodline from N** (1–8 hatched fish) opens a review that explains ancestry and standard similarity, shows the captured standard and asks for a unique name. The Family tab lists every line with this fish's ancestry and similarity side by side, plus the registry with rename.
...
After an explicit multi-day calendar advance, a **Calendar report** panel sits above the tank. Each tank with something to report gets a row: fish count, eggs laid or hatched, fish that became juvenile or adult, mean condition before and after, how many declined, the causes that limited condition with their day counts, what needs attention now, and an **Open** button for that tank. Quiet tanks are counted in one line. The panel closes with a statement that every decline had a named cause, or asks the player to export and report the save if one did not. **Dismiss** removes it. Closing and reopening the game never creates this report because absence no longer advances time.
...
The FS-503 slider editor is replaced. **Aquascape** in the tank controls, or **Open aquascape editor** in the Habitat tab, turns the live aquarium into the design surface: the HUD hides, a gold frame and badge mark edit mode, and the aquarium freezes: fish motion, plant sway and game time stop until Apply or Cancel, and time resumes from the moment editing began. Every piece has a grab area matching its drawing. Dragging moves it horizontally; it settles into the substrate (floating plants stay at the waterline), and releasing it where it would pinch swimming space slides it to the nearest valid spot. The selected piece shows a toolbar: smaller, larger, flip, new shape, duplicate and remove. Keyboard: arrows move and resize, F flips, R reshapes, Delete removes, Escape deselects.
